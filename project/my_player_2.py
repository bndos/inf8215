...
class MyAgent(Agent):
    ...
    def filter_wall_moves(self, wall_moves, state: Board, other_player):
        ...
        best_horizontal_distance = -1
        best_vertical_distance = -1

        best_horizontal_wall = None
        best_vertical_wall = None

        for wall_move in best_wall_moves:
            state_clone = state.clone()
            state_clone.play_action(wall_move, (other_player + 1) % 2)

            if wall_move[0] == 'WH':
                 if best_horizontal_distance <= state_clone.min_steps_before_victory(
                     other_player
                 ):
                     best_horizontal_distance = state_clone.min_steps_before_victory(
                         other_player
                     )

                     best_horizontal_wall = wall_move
            else:
                 if best_vertical_distance <= state_clone.min_steps_before_victory(
                     other_player
                 ):
                     best_vertical_distance = state_clone.min_steps_before_victory(
                         other_player
                     )
                     best_vertical_wall = wall_move


        best_wall_moves = []

        if best_horizontal_wall is not None:
            best_wall_moves.append(best_horizontal_wall)
        if best_vertical_wall is not None:
            best_wall_moves.append(best_vertical_wall)
